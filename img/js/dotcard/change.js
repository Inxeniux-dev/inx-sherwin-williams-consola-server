...
$(".btn-remision").click(function(){
    ...
    .then(function(data) {
      ...
      if(data.code == 201)
      {
          ...
          {
              output += `<option data-price = "${items[x]["precio"]}" data-desc = "${items[x]["codigo"]} ${items[x]["descripcion"]}"  value = "${items[x]["id"]}">${items[x]["codigo"]} - ${items[x]["descripcion"]}  [${items[x]["cantidad"]}]</option>`;
          }
...
$(".prods").change(function(){
      ...
      if(val > 0){ addTable(val, price, desc); }
      console.log(prods);
});


function addTable(val, price, desc)
{
  var descrip = "";
  let count = 0;
  $(".table-prods tbody tr").each(function(){
      descrip = $(this).find("td").eq(0).text().trim();
      if(descrip == desc.trim()){ count++; }
  });
...
function caltotal()
{
    ...
    if(total > 0){ $(".btn-change").removeClass("d-none"); }
    else { $(".btn-change").addClass("d-none"); }
}
...
$(".btn-change").click(function(){
    ...
    {
        data.append('remision', $("#remision").val());
    }
    else {
