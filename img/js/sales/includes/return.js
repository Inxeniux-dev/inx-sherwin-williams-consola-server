    ...
    if(err == 0)
    {
        ...
        .then(function(data) {
          ...
          { console.log("entro");
            if(data.status)
            {  console.log("entro 2");
                alertMessage("success", data.msg, "Aceptar", true, "../../detail/"+identified+"/");
                return;
            }
          }
          statusHTTP(data, "../../");
...
$(".btn-devolution").click(() => {
    $(".div-devolution").removeClass("d-none");

    $('html, body').animate({
        scrollTop: $(".div-devolution").offset().top
        }, 1500);
});
