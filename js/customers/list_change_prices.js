...
const generateAll = () => {
  ...
  Swal.fire({
    title: '¡Atención! ',
    input: 'text',
    text: "Al confirmar, se generará un documento con todos los productos y sus precios. Ingresa numero de sucural",    
    showCancelButton: true,
...
const confirmAll = (storeId) => {

  let url = `../../app/api/price.php?a=6`;
  const data = new FormData();
  data.append('storeId', storeId);

  loading_scrum('Generando documento, espere...');
  fetch(url,{method:'POST', body:data})
  .then(function(response) {return response.json();})
  .then(function(data) { console.log(data);
    const { code, error}  = data;
      ...
      if(code == 404)
      {
         showModalMessageError("warning", error, 2300);
         stop_scrum();
         return;
      }
      showModalMessageError("warning", "Información de precios no generada", 2300);
...
$("#upd-factor").click(() => {
    ...
    let factor =$("#factor");
    let error = 0;
    error += valid_imputs([factor]);
    error += valid_numeric_positive([factor]);
    if(error > 0 ){ showModalMessageError("warning", "¡Verifica los campos en rojo!", 2300); return; }


    Swal.fire({
        title: '¡Atención! ',
        text: "Al confirmar, se actualizarán todos los precios especiales automáticamente en relación al precio normal",
        type: 'warning',
        showCancelButton: true,
        confirmButtonColor: 'rgb(63 99 134)',
        cancelButtonColor: '#3085d6',
        confirmButtonText: '¡Si, actualizar factor!',
        cancelButtonText: 'No'
      }).then((result) => {
        if (result.value) {
              update_factor(factor.val());
        }
      });
...
function update_factor(factor)
{

  loading_scrum('Aplicando precio especial, espere...');
  let url = `../../app/api/config.php?a=1`;
  ...
  .then(function(data) { console.log(data);
    const { code }  = data;
      if(code == 201)
      {
         showModalMessageError("success", "Productos actualizados correctamente", 2300);
         setTimeout(function(){ window.location =`../listPrices/` }, 2300);
         return;
      }
      statusHTTP(data, "../../");
      stop_scrum();
