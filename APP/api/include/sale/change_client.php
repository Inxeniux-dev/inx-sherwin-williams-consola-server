<?php
    ...
    if($identified == null || $identified == 0 || $id_client == null || $id_client == 0)
    {
    ...
    if($action == 1)
    {
        if($id_client != 1)
        {
            echo json_encode(["code" => 200, "status" => false, "msg" => "El cliente es incorrecto", "error" => ["El cliente es incorrecto"]]); return;
        }
    }


    if($data->idtipo_de_venta != 1)
    {
        echo json_encode(["code" => 200, "status" => false, "msg" => "La venta es a crédito, no se puede realizar el cambio de cliente", "error" => ["La venta es a crédito, no se puede realizar el cambio de cliente"]]); return;
    }

    $res = $model->update_cliente($identified, $id_client);

    if(!$res)
    {
      echo json_encode(["code" => 200, "status" => false, "msg" => "El cliente no se ha actualizado",  "error" => ["El cliente no se ha actualizado"]]); return;
    }

    echo json_encode(["code" => 201, "status" => true, "msg" => "Cliente actualizado correctamente"]); return;
?>
