...
if(!$items){  echo json_encode(["code" => 200, "error" => ["Error al validar marca"]]); return; }

$items = $items->num_rows > 0 ? $items->fetch_object()->total : 0;

if($items > 0)
{
   echo json_encode(["code" => 200, "error" => ["La marca tiene productos asociados"]]); return;
}
...
$error = $response ? [] : ["Error al eliminar la marca"];
